...
[![Docker Repository on Quay](https://quay.io/repository/utilitywarehouse/ilo-exporter/status "Docker Repository on Quay")](https://quay.io/repository/utilitywarehouse/ilo-exporter)

The original repo is: https://github.com/infinityworks/hpilo-exporter
...
```
docker run -p 9416:9416 -e ilo_password=mypass hpilo-exporter:latest
```

You can then call the web server on the defined endpoint, `/metrics` by default.
```
curl 'http://127.0.0.1:9416/metrics?ilo_host=127.0.0.1&ilo_port=443&ilo_user=admin'
```

Passing argument to the docker run command
```
docker run -p 9416:9416 -e ilo_password=mypass hpilo-exporter:latest --port 9416 --ilo_user my_user
```
